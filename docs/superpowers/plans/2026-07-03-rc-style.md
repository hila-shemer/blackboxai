...
- [x] **Step 2: Full suite - menu goldens must pass unchanged**
...
- [x] **Step 3: Write the styled-menu system test (new golden)**
...
- [x] **Step 1: Write the failing knob test**
...
- [x] **Step 2: Run to verify the True case fails**
...
- [x] **Step 4: Audit the suite under the new default (the re-bless sweep)**
...
- [x] **Step 1: Write the failing updateRcKey tests**
...
TEST_CASE("reconfigure: rereads the rc, re-applies knobs, reports style fallback") {
  ...
  CHECK(server.currentStyle()->frameMetrics().handleHeight == 10);  // Gray's key 8 + border fold (see deviation note)
...
**Deviation (recorded at implementation):** the authored test asserted Gray's
`handleHeight == 8` (the raw key), but Task 8's loader folds the handle border
in (`8 + maxBw(handle)*2 = 10`) and its unit test pins exactly that
(`tests/unit/style_test.cc`, Gray case). Task 8's committed semantics win; the
retheme assertion was adapted to 10. No code changed.
